...
class MaeImagePretrainingConfig(FairseqDataclass):
    ...
    audio_mae: bool = field(
        default=False,
        metadata={
            "help": "if set, we use image_mae way to deal with audio files."
        },
    )
    sampling_rate: int = field(
        default=48000,
        metadata={"help": "sampling rate of input audio file."},
    )
    h5_format: bool = field(
        default=False,
        metadata={
            "help": "if set, dataset will read data file in h5df format."
        },
    )
    downsr_16hz: bool = field(
        default=False,
        metadata={
            "help": "if set, wav file's sample rate will be reduced to 16kHz."
        },
    )
    target_length: int = field(
        default=1024,
        metadata={
            "help": "This setting will pad the audio spectrogram with zeros."
        },
    )
    flexible_mask: bool = field(
        default=False,
        metadata={
            "help": "if true, we will using flexible inverse block mask method."
        },
    )
    ...
    AS2M_finetune: bool = field(
        default=False,
        metadata={
            "help": "if true, the task is to finetune model on Audioset 2M with weighted sample."
        },
    )
    spcv1_finetune: bool = field(
        default=False,
        metadata={
            "help": "if true, the task is to finetune model on speech commands v1  with weighted sample."
        },
    )
    roll_aug: bool = field(
        default=False,
        metadata={"help": "if true, we will use roll aug in fine-tuning."},
    )
    noise: bool = field(
        default=False,
        metadata={
            "help": "if true, we will add gaussian noise as augmentation during fine-tuning."
        },
    )
    weights_file: str = field(
        default="", metadata={"help": "the path of weighted sample file"}
    )
    num_samples: int = field(
        default=200000,
        metadata={
            "help": "this setting will determine the number of samples in each epoch, usually used in unbalanced training."
        },
    )
    is_finetuning: bool = field(
        default=False, metadata={"help": "this property has been deprecated"}
    )
...
class MaeImagePretrainingTask(FairseqTask):
    ...
    def load_dataset(
        ...
        self.datasets[split] = MaeImageDataset(
            ...
            yokogawa_eval=cfg.yokogawa_eval,
