...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Use fine-tuned EAT for inference in Audioset-eval"
    )
    parser.add_argument(
        "--eval_dir",
        type=str,
        help="Directory with eval.lbl and eval.tsv files",
        required=True,
    )
    parser.add_argument(
        "--label_file", help="Location of label files", required=True
    )
    parser.add_argument(
        "--model_dir",
        type=str,
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        help="Checkpoint for fine-tuned model",
        required=True,
    )
    parser.add_argument(
        "--target_length",
        type=int,
        help="Target length of Mel spectrogram in time dimension",
        required=True,
    )
    parser.add_argument(
        "--norm_mean",
        type=float,
        help="Mean value for normalization",
        default=-4.268,
    )
    parser.add_argument(
        "--norm_std",
    ...
    parser.add_argument(
        "--batch_size",
        type=int,
        help="Inference batch size in dataloader",
        default=16,
    )
    parser.add_argument(
        "--ap_log_path",
        type=str,
        default="EAT/ap_log.txt",
        help="Path to save the AP values log",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def load_audio_labels(eval_dir, set_name="test"):
    audio_paths = []
    labels = {}
    root_dir = ""

    with open(Path(eval_dir) / f"{set_name}.tsv", "r") as tsv_file:
        for line in tsv_file:
            ...
            if len(parts) == 2:
                ...
                root_dir = parts[0]
    with open(Path(eval_dir) / f"{set_name}.lbl", "r") as lbl_file:
    ...
    def __getitem__(self, idx):
        ...
        if sr != 16e3:
            source = torchaudio.functional.resample(
                source, orig_freq=sr, new_freq=16000
            ).float()
        ...
        source = (source - source.mean()) / (source.std() * 2)
        target = torch.zeros(len(self.vocab)).to(self.device)
        for lbl in label:
            if lbl in self.vocab:
                idx = self.vocab[lbl]
                target[idx] = 1
        return source, target, str(audio_path)
...
def main():
    ...
    audio_files = []
    with torch.no_grad():
        for sources, target, audio_file in tqdm(data_loader, desc="Testing"):
            pred = model(sources)
            pred = torch.sigmoid(pred)
            outputs.append(pred)
            targets.append(target)
            audio_files += list(audio_file)
    ...
    df_result = pd.DataFrame(
        {
            ...
            "label": targets_tensor.cpu().numpy()[:, 1],
            "predict": outputs_tensor.cpu().numpy()[:, 1],
        }
    )
    df_result.to_csv(os.path.join(args.output_dir, "test_results.csv"))
    mAP, ap_values = calculate_map(
        outputs_tensor.cpu().numpy(), targets_tensor.cpu().numpy(), inx_lbl
    )
    print(
        f"The fine-tuned model's performance on Audioset-eval with {len(dataset)} audio clips is: {mAP:.4f}"
    )
